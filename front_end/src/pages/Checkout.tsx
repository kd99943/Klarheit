      ...
      <form onSubmit={handleSubmit} className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-5">
          <div className="bg-brand-primary text-white rounded-2xl p-6 sm:p-8 shadow-xl sticky top-28">
            ...
            <div className="space-y-3 mb-8 text-sm">{activeLensTypes.map((type) => (<div key={type} className="flex justify-between text-white/70"><span>{LENS_LABELS[type] ?? type}</span><span>{formatPrice(LENS_PRICES[type] ?? 0)}</span></div>))}<div className="flex justify-between text-white/70 pt-2 border-t border-white/5"><span>{t("manifest.subtotal")}</span><span>{formatPrice(subtotal)}</span></div><div className="flex justify-between text-brand-cyan/80"><span>{t("manifest.complimentaryShipping")}</span><span>{formatPrice(0)}</span></div></div>
            <div className="mb-8 rounded-2xl bg-white/8 px-4 py-4 text-sm text-slate-200"><p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-white/50">{t("manifest.readiness")}</p><p className="mt-2">{completionScore >= 6 ? t("manifest.nearlyComplete") : t("manifest.stillInProgress")}</p></div>
            ...
            <div className="flex justify-between items-end mb-8 pt-6 border-t border-white/10"><span className="text-white/50 text-xs font-medium uppercase tracking-widest">{t("manifest.totalCommission")}</span><span className="text-3xl font-light tracking-tight">{formatPrice(subtotal)}</span></div>
