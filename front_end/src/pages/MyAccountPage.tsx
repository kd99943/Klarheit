...
const recentOrders = [
  {
    id: "PR-2401",
    frame: "Aero X1 Titanium",
    status: "Lens calibration",
    eta: "May 18",
    total: "$1,155.00",
  },
  {
...
const prescriptionStats = [
  { label: "OD Sphere", value: "-2.25" },
  { label: "OS Sphere", value: "-2.00" },
  { label: "PD", value: "63.5 mm" },
];
      ...
      <div className="mx-auto flex max-w-[1440px] flex-col gap-8">
        ...
        <div className="grid grid-cols-1 gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="flex flex-col gap-8">
            ...
            <SectionCard
              eyebrow="Optical Profile"
              title="Prescription Snapshot"
              description="Quick access to the measurement set currently used across checkout and configuration flows."
            >
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                {prescriptionStats.map((stat) => (
                  <div key={stat.label} className="rounded-2xl border border-slate-200 bg-slate-50/70 px-5 py-5">
                    <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-slate-400">{stat.label}</p>
                    <p className="mt-3 text-2xl font-display font-medium text-brand-primary">{stat.value}</p>
                  </div>
                ))}
              </div>
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                <Link to="/config-lab" className="w-full sm:w-auto">
                  <Button variant="outline-light" className="w-full">
                    Update Prescription
                  </Button>
                </Link>
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm text-slate-500">
                  Last calibration synced from Config Lab. Values shown here are display-only until you submit a new build.
                </div>
              </div>
            </SectionCard>
            ...
            >
              <div className="flex flex-col gap-4">
                {recentOrders.map((order) => (
                  <div key={order.id} className="rounded-2xl border border-slate-200 bg-white px-5 py-5 shadow-[0_18px_40px_-36px_rgba(15,23,42,0.3)]">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                      <div>
                        <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-slate-400">{order.id}</p>
                        <h3 className="mt-2 text-lg font-display font-medium text-brand-primary">{order.frame}</h3>
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:min-w-[360px]">
                        <div className="rounded-xl bg-slate-50 px-4 py-3">
                          <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-slate-400">ETA</p>
                          <p className="mt-2 text-sm font-medium text-brand-primary">{order.eta}</p>
                        </div>
                        <div className="rounded-xl bg-slate-50 px-4 py-3">
                          <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-slate-400">Total</p>
                          <p className="mt-2 text-sm font-medium text-brand-primary">{order.total}</p>
                        </div>
                        <div className="rounded-xl bg-slate-50 px-4 py-3 col-span-2 sm:col-span-1">
                          <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-slate-400">State</p>
